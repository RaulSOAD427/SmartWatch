...
class Pedometer:
  ...
  __jumps = 0        # the current step count
  ...
  __jump_thresh_low = 250   # Threshold from Tutorial 2
  __jump_thresh_high = 700 # Threshold from Tutorial 2
  ...
  def process(self):
    ...
    self.__b, self.__a = filt.create_filter(3, 1, "lowpass", self.__fs)   # Low-pass Filter Design
    ...
    count_steps, peaks_s = filt.count_peaks(lp,self.__thresh_low,self.__thresh_high)

    #Count the number of jumps
    count_jumps, peaks_j = filt.count_peaks(lp,self.__jump_thresh_low,self.__jump_thresh_high)
    ...
    return self.__steps, peaks_s, np.array(self.__filtered), self.__jumps, peaks_j
